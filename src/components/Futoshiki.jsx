...
export default function Futoshiki() {
  ...
  const initializeGrid = useCallback(() => {
    ...
    const horizontalConstraints = Array(gridSize)
      .fill(null)
      .map(() => Array(gridSize - 1).fill(null));
    const verticalConstraints = Array(gridSize - 1)
      .fill(null)
      .map(() => Array(gridSize).fill(null));
    const givenCells = Array(gridSize)
      .fill(null)
    ...
    if (DIFFICULTY_SETTINGS[difficulty].useConstraints) {
      for (let i = 0; i < gridSize; i++) {
        for (let j = 0; j < gridSize - 1; j++) {
          if (Math.random() < 0.3) {
            horizontalConstraints[i][j] = solution[i][j] < solution[i][j + 1] ? "<" : ">";
          }
          if (i < gridSize - 1 && Math.random() < 0.3) {
            verticalConstraints[i][j] = solution[i][j] < solution[i + 1][j] ? "<" : ">";
          }
        }
      }
    }
  ...
  const renderConstraint = (constraint, isVertical) => {
    ...
    const arrow = isVertical
      ...
      : (constraint === "<" ? <IconArrowNarrowLeft /> : <IconArrowNarrowRight />);
    return (
      <div style={{
        width: "20px",
        height: "20px",
